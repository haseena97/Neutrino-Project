...
! zion    = number of protons
      ...
      real*8             :: xmass(ionmax),ymass(ionmax), &
                            ...
                            spair,splas,sphot,sbrem,sreco,index,rad,temp,den,rm,m,templ,denl
...
! set the thermodynamic state
	open(unit=1,file='Cleandata.txt',status='old',action="readwrite")
     open(unit = 50, file='output_ds.txt') 
     write(50,*) 'snu','dsnudt','dsnudd','dsnuda','dsnudz'
    	DO v=1,135
	read(*,*)index,rad,templ,denl,rm,m
!	write(*,*)templ,denl
	temp=10**templ
	den=10**denl
!	print*,'temp',temp
...
! get the neutrino losses
      call sneut5(temp,den,abar,zbar, &
                  snu,dsnudt,dsnudd,dsnuda,dsnudz, &
...
!      write(6,*)
 	   write(50,02) snu, dsnudt, dsnudd, dsnuda, dsnudz
...
!                  'dsnuda=',dsnuda, &
!                  'dsnudz=',dsnudz
 02   format(1x,1pe11.4,1x,1pe11.4,1x,1pe11.4,1x, 1pe11.4,1x, 1pe11.4 )  !1x bagi ada sengkang
!      write(6,*)
      end do
...
! declare the pass
      real*8 :: temp,den,abar,zbar, &
                snu,dsnudt,dsnudd,dsnuda,dsnudz

! local variables
      real*8 :: spair,spairdt,spairdd,spairda,spairdz, &
      ...
      real*8 :: t9,xl,xldt,xlp5,xl2,xl3,xl4,xl5,xl6,xl7,xl8,xl9, &
...
! pair production
      real*8 :: rm,rmdd,rmda,rmdz,rmi,gl,gldt, &
...
! recomb
      real*8 :: ifermi12,zfermim12,nu,nudt,nudd,nuda,nudz, &
...
! numerical constants
      ...
      real*8, parameter :: fac1   = 5.0d0 * pi / 3.0d0, &
                           fac2   = 10.0d0 * pi, &
                           fac3   = pi / 5.0d0, &
                           oneth  = 1.0d0/3.0d0, &
                           twoth  = 2.0d0/3.0d0, &
...
! propagate through the routine. cv and ca are the vector and axial currents.
      ...
      real*8, parameter ::  theta  = 0.2319d0, &
                            xnufam = 3.0d0, &
...
! local variables
      ...
      real*8    :: an,a1(12),b1(12),a2(12),b2(12),rn,den,ff
...
! local variables
      integer  :: i,m1,k1,m2,k2
      real*8   :: an,a1(12),b1(12),a2(12),b2(12),rn,den,xx
